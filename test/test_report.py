...
class TestCompModFunction:
    ...
    def test_single_dataInputs(self):
        result = fr.compare_models(self.X, self.y, self.prtc_attr,
    ...
    def test_mixed_groupings(self):
        result = fr.compare_models([self.X, self.X],
    ...
    def test_with_protected_attributes(self):
        result = fr.compare_models([self.X, self.X], [self.y, self.y],
    ...
    def test_multiple_calls(self):
        ...
        _ = fr.compare_models(*args, flag_oor=False)
        result = fr.compare_models(*args, flag_oor=False)
        self.is_result_valid(result)
...
class TestCompModValidations:
    ...
    def test_mismatch_input_numbers(self):
        with pytest.raises(Exception):
            fr.compare_models({0: self.X, 1: self.X},
                                {0: self.y, 1: self.y},
                                {1: self.prtc_attr},
                                self.model_dict,
                                flag_oor=False)
    ...
    def test_invalid_y_member(self):
        with pytest.raises(Exception):
            fr.compare_models({0: self.X, 1: None},
                                {0: self.y, 1: self.y},
                                {0: self.prtc_attr, 1: self.prtc_attr},
                                self.model_dict,
                                flag_oor=False)
    ...
    def test_differing_keys(self):
        with pytest.raises(Exception):
            fr.compare_models({5: self.X, 6: self.X},
                                {0: self.y, 1: self.y},
                                {0: self.prtc_attr, 1: self.prtc_attr},
                                self.model_dict,
                                flag_oor=False)

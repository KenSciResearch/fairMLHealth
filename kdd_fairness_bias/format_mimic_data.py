'''
## NOTE: Pandas can directly read zipped csv files, i.e. df = pd.read_csv('filename.zip')
...
def custom_round(col, base=5, sig_dec=0):
    '''
        :arg sig_dec: number of significant decimals for the custom-rounded value
    '''
    assert base >= 0.01, (
...
class mimic_loader():
    def __init__(self, data_file=""):
        ...
        self.data_dir = os.path.dirname(self.output_file)
        if self.data_dir != "":
    ...
    def __load_mimic_data(self, data_key):
        ...
        data_file = f"{self.data_dir}/zipped_files/{file_dict[data_key]}.csv.gz"
    ...
    def load_dxpx_data(self, feature_type):
        ''' Returns a pandas dataframe with formatted diagnosis information
        '''
        ftypes = ['dx','px']
        ...
        ohe_df = pd.get_dummies(df[f'{feature_type}_CCS'], prefix=f'{prefix_dict[feature_type]}_CCS')

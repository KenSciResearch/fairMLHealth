...
def __preprocess_stratified(X, y_true, y_pred=None, y_prob=None,
                            features:list=None, verbose:bool=True):
    """
    ...
    Requirements:
        ...
        to run the assessment. If any data are not discrete and there are more than
        11 values, the reporter will reformat those data into quantiles
    """
    #
    max_cats = 11
    xtiles = 5
    ...
    for f in stratified_features:
        ...
        #
        if df[f].nunique() > max_cats and not df[f].astype(str).str.isdigit().all():
            if verbose:
                print(f"\t{f} has more than {max_cats} values.",
                      "Discretizing to {xtiles} buckets.")
            not_null = df[f].notnull()
            qtl = pd.qcut(df.loc[not_null, f].values, xtiles, duplicates='drop')
            df.loc[not_null, f] = qtl.codes + 1
            df[f] = "quantile_" + df[f].astype(str)
...
def data_report(X, y_true, features:list=None, verbose:bool=True):
    """
    ...
    if features is None:
        features = list(X)
    df = __preprocess_stratified(X, y_true, features=features, verbose=verbose)
    ...
    rprt.loc[rprt['FEATURE VALUE'].ne("nan"), 'VALUE_PROMINENCE'] = \
        (rprt['N OBS']/rprt['N_MISSING'].apply(lambda x: N_obs - x))
    rprt.loc[rprt['FEATURE VALUE'].eq("nan"), 'VALUE_PROMINENCE'] = \
        (rprt['N OBS']/N_obs)
    #
    rprt = rprt.round(4)
    return rprt
...
def regression_performance(X, y_true, y_pred, features:list=None, verbose:bool=True):
    """
    Generates a table of stratified performance metrics for each specified feature
...
def regression_fairness(X, y_true, y_pred, features:list=None, verbose:bool=True):
    """
    Generates a table of stratified fairness metrics metrics for each specified feature
    ...
    for i, row in rprt.iterrows():
        ...
        v = row['FEATURE VALUE']
        df['prtc_attr'] = 0
        df.loc[df[f].eq(v), 'prtc_attr'] = 1
        if v != "nan":
            df.loc[df[f].eq("nan"), 'prtc_attr'] = np.nan
        subset = df.loc[df['prtc_attr'].notnull(), ]
        try:
            r2 = reports.regression_fairness(subset, subset['prtc_attr'],
                                             subset[y], subset[yh],
                                             priv_grp=1, sig_dec=6,
                                             skip_individual=True)
            r2 = r2.droplevel(0).rename(columns={'Value':i}).transpose()

        except BaseException as e:
            r2 = pd.DataFrame({'Fairness_Notes':[e]})
        r2['FEATURE'] = f
        r2['FEATURE VALUE'] = v
        res_f.append(r2)
    rprt_update = pd.concat(res_f, ignore_index=True)
    if 'Prevalence of Privileged Class (%)' in list(rprt_update):
        rprt_update.drop("Prevalence of Privileged Class (%)", axis=1, inplace=True)
...
def classification_fairness(X, y_true, y_pred, features:list=None,
    ...
    for f in stratified_features:
        ...
        for v in vals:
            ...
            subset = df.loc[df[pa_name].notnull(), :].copy().set_index(pa_name)
            try:
                meas = __cf_group(subset, subset[y], subset[yh],
                                  prtc_attr_name=pa_name, priv_grp=1)
                r = pd.DataFrame(meas, index=[0])
            except BaseException as e:
                r = pd.DataFrame({'Fairness_Notes': [e]})
            r['FEATURE'] = f
            r['FEATURE VALUE'] = v
            res.append(r)
    full_res = pd.concat(res, ignore_index=True)
...
def __cf_group(X, y_true, y_pred, prtc_attr_name, priv_grp=1):
    pa_name = prtc_attr_name
    gf_vals = {}
    gf_vals['N OBS'] =  X.shape[0]
    gf_vals['PPV Ratio'] = \

...
def performance(
    ...
    if pred_type == "classification":
        df = __strat_class_performance(
            X, y_true, y_pred, y_prob, features, add_overview
        )
    elif pred_type == "regression":
...
def __classification_summary(
    *, X, prtc_attr, y_true, y_pred, y_prob=None, priv_grp=1, **kwargs
    ...
    def update_summary(summary_dict, pa_name, y_true, y_pred, y_prob, priv_grp):
        ...
        for k in drop_keys:
            summary_dict.pop(k)
        summary_dict["Equalized Odds Difference"] = eq_odds_diff(
            y_true, y_pred, prtc_attr=pa_name
        )
        summary_dict["Equalized Odds Ratio"] = eq_odds_ratio(
            y_true, y_pred, prtc_attr=pa_name
        )
...
def __strat_class_performance(
    X, y_true, y_pred, y_prob=None, features: list = None, add_overview=True
):
...
def __strat_reg_performance(
    X, y_true, y_pred, features: list = None, add_overview=True
):
    """
    Generates a table of stratified performance metrics for each specified
    feature
...
def __sort_table(strat_tbl):
    ...
    head_names = ["Feature Name", "Feature Value", "Obs.", f"{_y} Mean", f"{_yh} Mean"]
